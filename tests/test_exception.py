...
from tests.fixtures_test import async_client_fix
...
async def test_login_ex(login_data, async_client_fix):
    ...
    if response.status_code == 404:
       ...
       assert resp_json["er_code"] == "Wrong user_name -> login"

    if response.status_code == 401:
        assert resp_json["message"] == "Authorization failed"
#pytest test_exception.py::test_login_ex   ->   to start test
...
async def test_select_data_ex(select_data, async_client_fix):
    ...
    assert resp.status_code == 200  #not bad req, just nothing found
    data_resp = resp.json()
    assert data_resp["message"] == "No data for your request"
    assert data_resp["detail"] == "Nothing was found in the base"
#pytest test_exception.py::test_select_data_ex    ->   to start test
...
#pytest test_exception.py::test_delete_ex    -> to start test
...
async def test_create_data_ex(async_client_fix, create_data, login_data):
    ...
    create_req = await as_client.post("/create", json=create_data, headers={"Authorization": f"Bearer {token}"})
    ...
    if create_req.status_code == 403:
        if create_resp_json["er_code"] == "Role creating -> create":
            assert create_resp_json["message"] == "You can't create role 'admin' or 'moder', if you are 'user'"

        elif create_resp_json["er_code"] == "Not enter to endpoint -> auth token":
            assert create_resp_json["detail"] == "Not enough permissions"
#pytest test_exception.py::test_create_data_ex -> to start test


@pytest.mark.parametrize(
    "update_data, login_data, id",
    [
        (
            {},  #need empty json for patch req, and admin_token
            {"user_name": "admin_two", "password": "admin456"},
            7
        ),
        (   #not empty data, but user_token
            {"user": "guest_two", "title": "Lead", "description": "Limited access", "completed": 0, "role": "moderator", "password": "q3w8e1"},
            {"user_name": "guest_three", "password": "guest_three_18"},
            9
        ),
...
@pytest.mark.anyio
async def test_update_data_ex(async_client_fix, update_data, login_data, id):
    as_client = async_client_fix
    login_req = await as_client.post("/login", json=login_data)
    token = login_req.json()["token"]

    update_req = await as_client.patch(f"/update/{id}", json=update_data, headers={"Authorization": f"Bearer {token}"})
    update_resp_json = update_req.json()
    assert update_req.status_code in [400, 403, 404]

    if update_req.status_code == 400:
        assert update_resp_json["er_code"] == "Empty req -> update"
        assert update_resp_json["detail"] == "Your body request is empty"

    elif update_req.status_code == 403:
        assert update_resp_json["er_code"] == "Not enter to endpoint -> auth token"
        assert update_resp_json["detail"] == "Not enough permissions"

    elif update_req.status_code == 404:
        assert update_resp_json["er_code"] == "No id in base -> update"

#pytest test_exception.py::test_update_data_ex -x
